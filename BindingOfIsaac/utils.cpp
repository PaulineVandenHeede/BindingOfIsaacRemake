...
bool utils::IsRectInRect(const Rectf & r1, const Rectf & r2)
{
	...
	if (r1.bottom < r2.bottom)
		inSquare = false;
	if ((r1.bottom + r1.height) > (r2.bottom + r2.height))
		inSquare = false;
...
std::string utils::GetAttributeValue(const std::string& attrName, const std::string& element)
{
	...
	size_t pos = element.find(attrName + "=\""); 
	
	if (pos == std::string::npos) // when it didn't find anything return empty string
		return attribute;

	pos += attrName.size() + 2;
	
	size_t length = element.find("\"", pos) - pos;
	attribute = element.substr(pos, length);

	return attribute;
}

std::string utils::GetPartValue(const std::string& partName, const std::string& element)
{
	std::string part{};
	...
	size_t pos = element.find(begin.str());
	
	if (pos == std::string::npos) // when it didn't find anything return empty string
		return part;

	pos += begin.str().size();
	size_t length = element.find(end.str(), pos) - pos;
	part = element.substr(pos, length);

	return part;
}

Point2f utils::StringToPos(const std::string & pos)
{
	Point2f temp{};
...
std::pair<int, int> utils::StringToHealthBar(const std::string & healthBar)
{
	std::pair<int, int> temp{};

	std::stringstream hbCopy{ healthBar };
	std::string part{};
	std::getline(hbCopy, part, ',');
