...
#pragma region ReadFromFile
	...
	std::string GetPartValue(const std::string& partName, const std::string& element);

	Point2f StringToPos(const std::string & pos);
	std::pair<int, int> StringToHealthBar(const std::string & healthBar);
